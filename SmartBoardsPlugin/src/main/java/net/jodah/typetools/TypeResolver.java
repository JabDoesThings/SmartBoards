...
public final class TypeResolver {
   ...
   *
   * @param type to onResolve argument for
   ...
   * returned.
   *
   * @param genericType to onResolve argument for
   * @param subType to extract type variable information from
   ...
   * returned.
   *
   * @param type to onResolve arguments for
   * @param subType to extract type variable information from
   ...
   * null} is returned.
   *
   * @param genericType to onResolve arguments for
   * @param subType to extract type variable information from
   ...
   *
   * @param type to onResolve generic type for
   * @param subType to extract type variable information from
   ...
   *
   * @param genericType to onResolve raw class for
   * @param subType to extract type variable information from
  ...
  private static Class<?> resolveRawClass(
      Type genericType, Class<?> subType, Class<?> functionalInterface) {
    if (genericType instanceof Class) {
      return (Class<?>) genericType;
    } else if (genericType instanceof ParameterizedType) {
      return resolveRawClass(
          ((ParameterizedType) genericType).getRawType(), subType, functionalInterface);
    } else if (genericType instanceof GenericArrayType) {
      GenericArrayType arrayType = (GenericArrayType) genericType;
      Class<?> component =
          resolveRawClass(arrayType.getGenericComponentType(), subType, functionalInterface);
  ...
  private static Map<TypeVariable<?>, Type> getTypeVariableMap(
      final Class<?> targetType, Class<?> functionalInterface) {
  ...
  private static void populateSuperTypeArgs(
      final Type[] types, final Map<TypeVariable<?>, Type> map, boolean depthFirst) {
    for (Type type : types) {
  ...
  private static void populateTypeArgs(
      ParameterizedType type, Map<TypeVariable<?>, Type> map, boolean depthFirst) {
  ...
  private static void populateLambdaArgs(
      Class<?> functionalInterface, final Class<?> lambdaType, Map<TypeVariable<?>, Type> map) {
    if (RESOLVES_LAMBDAS) {
      // Find SAM
      for (Method m : functionalInterface.getMethods()) {
        if (!isDefaultMethod(m) && !Modifier.isStatic(m.getModifiers()) && !m.isBridge()) {
          ...
          if (objectMethod != null
              && Arrays.equals(m.getTypeParameters(), objectMethod.getTypeParameters())) continue;
          ...
          for (int i = 0; i + argOffset < arguments.length; i++) {
            if (paramTypeVars[i] instanceof TypeVariable)
              map.put(
                  (TypeVariable<?>) paramTypeVars[i + paramOffset],
