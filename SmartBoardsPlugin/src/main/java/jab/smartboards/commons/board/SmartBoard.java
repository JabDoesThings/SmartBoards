...
public interface SmartBoard extends BoardSizable {
  ...
  default ItemFrame createItemFrame(
    ...
    if (keepChunksLoaded()) {
      if (!chunk.getPluginChunkTickets().contains(PluginSmartBoards.instance)) {
        chunk.addPluginChunkTicket(PluginSmartBoards.instance);
      }
      if (!chunk.isLoaded()) {
        chunk.load();
      }
    ...
    if (itemFrame == null) {
      System.out.println("Creating a new ItemFrame at {x: " + x + ", y: " + y + ", z: " + z + "}");
