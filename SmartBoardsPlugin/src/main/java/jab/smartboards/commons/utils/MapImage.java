...
public class MapImage {
   ...
   * @param color The color to draw on the initial image.
   */
  public MapImage(int width, int height, byte color) {
    setWidth(width);
    setHeight(height);
    this.bytes = new byte[width * height];
    // Create a blank image.
  ...
  public PacketPlayOutMap createPacket(short index) {
    ...
    byte b = (byte) 0;
    boolean c = true;
    packet = new PacketPlayOutMap(index, b, c, false, new ArrayList<>(), bytes, 0, 0, 128, 128);
  ...
  public boolean isSent(@NotNull Player player) {
    ...
    Map<Integer, Long> mapTimes = SmartBoards.mapMapPacketTimes.get(player.getUniqueId());
    if (mapTimes != null) {
      int id = getPacketId();
  ...
  public void send(@NotNull Player player) {
    ...
    Map<Integer, Long> mapTimes =
        SmartBoards.mapMapPacketTimes.computeIfAbsent(playerId, k -> new HashMap<>());
